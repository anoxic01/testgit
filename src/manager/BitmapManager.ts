module manager {
	export class BitmapManager {
		...
		/** 筹码位图 **/
		private m_dicChips			:	Dictionary;
		private m_dicGameChips			:	Dictionary;					//下注筹码
		/** 筹码阴影 **/
		public bmpChipShadow			:	egret.Bitmap;
		
		/** 珠盘路子 **/
		private m_dicBeads			:	Dictionary;
		
		/** 桌子图标 **/
		private m_dicTableIcon			:	Dictionary;
		
		/**	语言位图 **/
		private m_aLanguage			:	string[];
		...
		/** 扑克 **/
		private m_dicCard			:	Dictionary;
		/** 小扑克 **/
		private m_dicCardSmall			:	Dictionary;
		/** 大扑克 **/
		private m_dicCardBig			:	Dictionary;
		
		/** 多桌过渡 **/
		private m_dicMultiTrans			:	Dictionary;
		
		/** 桌子背景 **/
		private m_aTableBg			:	egret.Bitmap[];
		public aTableBgHover			:	egret.Bitmap[];
		private m_dicMultitableBg		:	Dictionary;
		private m_dicMultitableFillBg		:	Dictionary;
		
		/** 默认桌 **/
		private m_dicTable			:	Dictionary;
		
		private m_mobile			:	Dictionary;
...
//			this.initChip();
		...
		public initMobileApp():void
		{
			...
			{
				this.m_mobile[define.MobileDefine.LINK_IOS+(i+1)] = MobileAppManager.getInstance().createBitmapByName(define.MobileDefine.LINK_IOS+(i+1));
				num = MobileAppManager.getInstance().createBitmapByName(define.MobileDefine.LINK_NUM+(i+1));
				// this.m_mobile[define.MobileDefine.LINK_NUM+(i+1)] = MovieclipFrameToBitmapData.draw(num);
				if(i<define.MobileDefine.AND_COUNT)
				{
					this.m_mobile[define.MobileDefine.LINK_AND+(i+1)] = MobileAppManager.getInstance().createBitmapByName(define.MobileDefine.LINK_AND+(i+1));
				}
			}
		}
		...
		private initBead():void
		{
			...
			for (var i:int = 0; i <= 36; i++) 
			{
				...
				var color:uint = RouData.getInstance().getColor(i);
		...
		private initLanguage():void
		{
			...
			this.addBmpdLanguage(language.Language.sBitmapdataTest, new BitmapData(30,10));
...
//			this.addBmpdLanguage_2(Language.sBitmapdataJB,[[new JB_Default_CN_Asset(), new JB_Default_TW_Asset(), new JB_Default_TW_Asset()],[new JB_Rollover_CN_Asset(), new JB_Rollover_TW_Asset(), new JB_Rollover_TW_Asset()]]);
			this.addBmpdLanguage_2(language.Language.sBitmapdataDZ,[[ResourceManager.getInstance().createBitmapByName("Theme_DZ_Default_CN_Asset") , ResourceManager.getInstance().createBitmapByName("Theme_DZ_Default_TW_Asset") , ResourceManager.getInstance().createBitmapByName("Theme_DZ_Default_TW_Asset") ],[ResourceManager.getInstance().createBitmapByName("Theme_DZ_Rollover_CN_Asset") , ResourceManager.getInstance().createBitmapByName("Theme_DZ_Rollover_TW_Asset") , ResourceManager.getInstance().createBitmapByName(define.Define.SWF_THEME,"Theme_DZ_Rollover_TW_Asset") ]]);
//			this.addBmpdLanguage_2(Language.sBitmapdataDT,[[new DT_Default_CN_Asset(), new DT_Default_TW_Asset(), new DT_Default_TW_Asset()],[new DT_Rollover_CN_Asset(), new DT_Rollover_TW_Asset(), new DT_Rollover_TW_Asset()]]);
			...
			this.addBmpdLanguage_3(language.Language.sGoodRoadWait, [ResourceManager.getInstance().createBitmapByName("Table_Hint_Wait_Asset_CN"), ResourceManager.getInstance().createBitmapByName("Table_Hint_Wait_Asset_CN"), ResourceManager.getInstance().createBitmapByName("Table_Hint_Wait_Asset_EN")]);"Table_Hint_FailGamel_Asset_TW"), ResourceManager.getInstance().createBitmapByName("Table_Hint_FailGamel_Asset_EN")]);
			...
			addBmpdLanguage_3(language.Language.sLose, [ResourceManager.getInstance().createBitmapByName("Table_Hint_Lose_Asset_CN"), ResourceManager.getInstance().createBitmapByName("Table_Hint_Lose_Asset_TW"), ResourceManager.getInstance().createBitmapByName("Table_Hint_Lose_Asset_EN")]);
		...
		private addBmpdLanguage( _sKey:string, _bmpd:egret.BitmapData):void{
			var arr : Array = BitmapLanguageUtil.cut(_bmpd,3);
		 ...
		 * 
		 */		
		private addBmpdLanguage_2(_sKey:String, _arr:Array):void{
		...
		private addBmpdLanguage_3( _sKey:string, _arr:Array):void{
		...
		private initCard():void{
			...
			this.m_dicCardSmall["BB"] = BitmapUtil.snapshot(ResourceManager.getInstance().createBitmapByName("CardX_BB"));
			this.m_dicCardSmall["Bg"] = BitmapUtil.snapshot(ResourceManager.getInstance().createBitmapByName("CardX_Bg"));
			...
			for (var i:int = 0; i < 4; i++) 
			{
				for (var j:int = 0; j < 13; j++) 
				{
					switch(j){
						case 0:
							card = ResourceManager.getInstance().createBitmapByName(Define.SWF_CARD,"Card_"+strA+arr[i]);
							...
							break;
						case 9:
							card = ResourceManager.getInstance().createBitmapByName(Define.SWF_CARD,"Card_"+strT+arr[i]);
						...
						case 10:
							...
							break;
						case 11:
							card = ResourceManager.getInstance().createBitmapByName(Define.SWF_CARD,"Card_"+strQ+arr[i]);
							...
							break;
						case 12:
							card = ResourceManager.getInstance().createBitmapByName(Define.SWF_CARD,"Card_"+strK+arr[i]);
