module packet.pack_lobby {
	export class S_Lobby_Update_Table_Pkt implements iface.IProtocolStruct{
		public Type			:	number;
		public SN			:	number;
//		 private tableList:<SimpleTable>;
		...
		public execute( _oData ):void
		{
			...
			this.Type	=	_oData.Type;
			this.SN		=	_oData.SN;
			var _iLen  = _oData.TableList.length;
			var _tableStruct ;
						
			for (var i:number= 0; i < _iLen; i++) 
			{
				
				_tableStruct = lobby.model.LobbyData.getInstance().lobbyInfo.findTableStructByTT(_oData.TableList[i].ThemeID, _oData.TableList[i].TableID);
...
//					}
					...
					if (old==false && _tableStruct.IsOffline==true){
						manager.LobbyManager.getInstance().offlineTable(_tableStruct);
					}
