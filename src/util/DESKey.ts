module util {
	export class DESKey implements ISymmetricKey{
		...
		private static  Df_Key = [ 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32,
				0x10, 0x89, 0xab, 0xcd, 0xef, 0x01, 0x23, 0x45, 0x67 ];
	
		private static  bytebit = [ 128, 64, 32, 16, 8, 4, 2, 1 ];
	
		private static  bigbyte = [ 0x800000, 0x400000, 0x200000, 0x100000, 0x80000, 0x40000, 0x20000, 0x10000, 0x8000,
		...
		private static  pc1 = [ 56, 48, 40, 32, 24, 16, 8, 0, 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2,
				59, 51, 43, 35, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 60, 52, 44, 36, 28, 20, 12,
				4, 27, 19, 11, 3 ];
	
		private static  totrot = [ 1, 2, 4, 6, 8, 10, 12, 14, 15, 17, 19, 21, 23, 25, 27, 28 ];
	
		private static  pc2 = [ 13, 16, 10, 23, 0, 4, 2, 27, 14, 5, 20, 9, 22, 18, 11, 3, 25, 7, 15, 6, 26, 19, 12, 1, 40,
				51, 30, 36, 46, 54, 29, 39, 50, 44, 32, 47, 43, 48, 38, 55, 33, 52, 45, 41, 49, 35, 28, 31 ];
	
		private static  SP1 = [ 0x01010400, 0x00000000, 0x00010000, 0x01010404, 0x01010004, 0x00010404, 0x00000004,
		...
		private static  SP2 = [ 0x80108020, 0x80008000, 0x00008000, 0x00108020, 0x00100000, 0x00000020, 0x80100020,
		...
		private static  SP3 = [ 0x00000208, 0x08020200, 0x00000000, 0x08020008, 0x08000200, 0x00000000, 0x00020208,
		...
		private static  SP4 = [ 0x00802001, 0x00002081, 0x00002081, 0x00000080, 0x00802080, 0x00800081, 0x00800001,
		...
		private static  SP5 = [ 0x00000100, 0x02080100, 0x02080000, 0x42000100, 0x00080000, 0x00000100, 0x40000000,
		...
		private static  SP6 = [ 0x20000010, 0x20400000, 0x00004000, 0x20404010, 0x20400000, 0x00000010, 0x20404010,
		...
		private static  SP7 = [ 0x00200000, 0x04200002, 0x04000802, 0x00000000, 0x00000800, 0x04000802, 0x00200802,
		...
		private static  SP8 = [ 0x10001040, 0x00001000, 0x00040000, 0x10041040, 0x10000000, 0x10001040, 0x00000040,
		...
		{
			var i:number=0;
			for (i=0;i<this.encKey.length;i++) { this.encKey[i]=0; }
			for (i=0;i<this.decKey.length;i++) { this.decKey[i]=0; }
			this.encKey=null;
			this.decKey=null;
		 ...
		 */
		protected generateWorkingKey(encrypting:Boolean, key :egret.ByteArray, off:number):any[]
		{
			//int[] newKey = new int[32];
			var newKey = [];
			//boolean[] pc1m = new boolean[56], pcr = new boolean[56];
			var pc1m :egret.ByteArray = new egret.ByteArray;
			var pcr :egret.ByteArray = new egret.ByteArray;
			
			var l:number;
	
			for (var j:number = 0; j < 56; j++)
			{
				l = DESKey.pc1[j];
	
				pc1m[j] = ((key[off + (l >>> 3)] & DESKey.bytebit[l & 0x07]) != 0);
			}
		 ...
		 * the DES engine.
		 */
		protected desFunc(wKey, inp :egret.ByteArray, inOff:number, out :egret.ByteArray, outOff:number):void
		{
			var work:number;
			var right:number;
			var left:number;
			...
			for (var round:number = 0; round < 8; round++)
			{
				var fval:number;
	
				work = (right << 28) | (right >>> 4);
				work ^= wKey[round * 4 + 0];
				fval = DESKey.SP7[work & 0x3f];
				fval |= DESKey.SP5[(work >>> 8) & 0x3f];
				fval |= DESKey.SP3[(work >>> 16) & 0x3f];
				fval |= DESKey.SP1[(work >>> 24) & 0x3f];
				work = right ^ wKey[round * 4 + 1];
				fval |= DESKey.SP8[work & 0x3f];
				fval |= DESKey.SP6[(work >>> 8) & 0x3f];
				fval |= DESKey.SP4[(work >>> 16) & 0x3f];
				fval |= DESKey.SP2[(work >>> 24) & 0x3f];
				left ^= fval;
				work = (left << 28) | (left >>> 4);
				work ^= wKey[round * 4 + 2];
				fval = DESKey.SP7[work & 0x3f];
