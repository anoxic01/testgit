module util {
	export class BlowFishKey implements ISymmetricKey {

		private static  KP = [ 0x243F6A88, 0x85A308D3, 0x13198A2E, 0x03707344, 0xA4093822, 0x299F31D0,
			0x082EFA98, 0xEC4E6C89, 0x452821E6, 0x38D01377, 0xBE5466CF, 0x34E90C6C, 0xC0AC29B7, 0xC97C50DD, 0x3F84D5B5,
			0xB5470917, 0x9216D5D9, 0x8979FB1B ];

		private static  KS0 = [ 0xD1310BA6, 0x98DFB5AC, 0x2FFD72DB, 0xD01ADFB7, 0xB8E1AFED, 0x6A267E96, 0xBA7C9045, 0xF12C7F99, 0x24A19947,
		...
		private static  KS1 = [ 0x4B7A70E9, 0xB5B32944, 0xDB75092E, 0xC4192623, 0xAD6EA6B0, 0x49A7DF7D, 0x9CEE60B8, 0x8FEDB266, 0xECAA8C71,
		...
		private static  KS2 = [ 0xE93D5A68, 0x948140F7, 0xF64C261C, 0x94692934, 0x411520F7, 0x7602D4F7, 0xBCF46B2E, 0xD4A20068, 0xD4082471,
		...
		private static  KS3 = [ 0x3A39CE37, 0xD3FAF5CF, 0xABC27737, 0x5AC52D1B, 0x5CB0679E, 0x4FA33742, 0xD3822740, 0x99BC9BBE, 0xD5118E9D,
		...
		private static  ROUNDS:number= 16;
		private static  BLOCK_SIZE:number= 8; // bytes = 64 bits
		private static  SBOX_SK:number= 256;
		private static  P_SZ:number= BlowFishKey.ROUNDS + 2;
	
		private  S0;
		private  S1;
		private  S2;
		private  S3; // the s-boxes
		private  P; // the p-array
	
		private  key:egret.ByteArray = null;
		...
		{
			var i :number=0;
			for (i=0;i<this.S0.length;i++) { this.S0[i]=0; }
			for (i=0;i<this.S1.length;i++) { this.S1[i]=0; }
			for (i=0;i<this.S2.length;i++) { this.S2[i]=0; }
			for (i=0;i<this.S3.length;i++) { this.S3[i]=0; }
			for (i=0;i<this.P.length;i++) { this.P[i]=0; }
			this.S0 = null;
			this.S1 = null;
			this.S2 = null;
			this.S3 = null;
		 ...
		 */
		private processTable(xl :number, xr :number, table):void
		{
			var size:number= table.length;
	
			for (var s:number= 0; s < size; s += 2)
			{
				xl ^= this.P[0];
	
				for (var i:number= 1; i < BlowFishKey.ROUNDS; i += 2)
				{
					xr ^= this.F(xl) ^ this.P[i];
					xl ^= this.F(xr) ^ this.P[i + 1];
				}
	
				xr ^= this.P[BlowFishKey.ROUNDS + 1];
		...
		private setKey(key:egret.ByteArray):void
		{
			/*
			 ...
			 */
			this.S0 = BlowFishKey.KS0.concat();
			this.S1 = BlowFishKey.KS1.concat();
			this.S2 = BlowFishKey.KS2.concat();
			this.S3 = BlowFishKey.KS3.concat();
			this.P = BlowFishKey.KP.concat();
			...
			for (var i:number= 0; i < BlowFishKey.P_SZ; i++)
			{
				// get the 32 bits of the key, in 4 * 8 bit chunks
				var data:number= 0x0000000;
			...
			this.processTable(this.P[BlowFishKey.P_SZ - 2], this.P[BlowFishKey.P_SZ - 1], this.S0);
			this.processTable(this.S0[BlowFishKey.SBOX_SK - 2], this.S0[BlowFishKey.SBOX_SK - 1], this.S1);
			this.processTable(this.S1[BlowFishKey.SBOX_SK - 2], this.S1[BlowFishKey.SBOX_SK - 1], this.S2);
			this.processTable(this.S2[BlowFishKey.SBOX_SK - 2], this.S2[BlowFishKey.SBOX_SK - 1], this.S3);
		}
		 ...
		 */
		private encryptBlock(src:egret.ByteArray, srcIndex :number, dst:egret.ByteArray, dstIndex :number):void
		{
		 ...
		 * exact multiple of our blocksize.
		 */
		private decryptBlock(src:egret.ByteArray, srcIndex :number, dst:egret.ByteArray, dstIndex :number):void
		{
			var xl:number= this.BytesTo32bits(src, srcIndex);
			var xr:number= this.BytesTo32bits(src, srcIndex + 4);
	
			xl ^= this.P[BlowFishKey.ROUNDS + 1];
	
			for (var i:number= BlowFishKey.ROUNDS; i > 0; i -= 2)
			{
				xr ^= this.F(xl) ^ this.P[i];
				xl ^= this.F(xr) ^ this.P[i - 1];
			}
		...
		private BytesTo32bits(b:egret.ByteArray, i :number) :number
		{
			return ((b[i] & 0xff) << 24) | ((b[i + 1] & 0xff) << 16) | ((b[i + 2] & 0xff) << 8) | ((b[i + 3] & 0xff));
		}
	
		private Bits32ToBytes(i :number, b:egret.ByteArray, offset :number):void
		{
			b[offset + 3] = i;
			b[offset + 2] = (i >> 8);
			b[offset + 1] = (i >> 16);
