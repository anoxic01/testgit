module lobby.view.quick {
	export class QuickTableRou extends QuickTable{
//		private this.m_mcAsset		:	*;								//房间资源
...
//		private m_btnLimit		:	ButtonMcLanguage;				//限红选择
		...
		public constructor() {
			...
			this.m_road = new route.quick.QuickTableRoadMapRou(this.m_mcAsset);
...
//			}
		 ...
		 public setData(_tableStruct):void{
			...
			switch(this.m_struct.TableType){
				...
				case define.Define.TABLE_TYPE_ROBOT:
					this.m_mcTableName = manager.ResourceManager.getInstance().getInstanceByNameFromDomain(define.Define.SWF_TABLE,"Table_Label_Rou_Asset");
			...
			//			this.m_mcAsset.mc_3.x = (this.m_mcAsset.mc_2.x + this.m_mcAsset.mc_2.width);
		 ...
		 public onChangeLanguage():void{
			...
			this.m_mcAsset.tf_label.text = "";//manager.LobbyManager.getInstance().getLanguageString(language.Language.sGame_Name_Rou) + String(this.m_struct.TableID) + manager.LobbyManager.getInstance().getLanguageString(language.Language.sTable);
...
//					}else{
//						playerTableOwnStatusStruct = model.LobbyData.getInstance().getPlayerTableOwnStatusStruct(this.m_struct.TableID);
//						if(playerTableOwnStatusStruct){
//							this.m_struct.BetLimitID = playerTableOwnStatusStruct.CurrBetLimitID;
...
//							});
//						}else{
//							this.m_struct.joinTableType = JoinTableType.SINGEL;
